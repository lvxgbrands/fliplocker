...
export default async function SimulatorCheckoutPage({
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fa] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-lg border border-slate-200 overflow-hidden">
        <div className="bg-[#003087] px-6 py-4 flex items-center justify-between">
          ...
          <span className="rounded bg-amber-400 px-2 py-0.5 text-[10px] font-bold text-slate-900 uppercase tracking-wide">
            Sandbox simulator
          </span>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <p className="text-xs text-slate-400">Paying FlipLocker deal {deal.shortCode}</p>
            <p className="font-semibold text-slate-800">{cardTitle(deal)}</p>
          </div>
