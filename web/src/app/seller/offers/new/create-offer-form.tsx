...
async function uploadViaPresign(file: File): Promise<string> {
  const presign = await fetch("/api/uploads/presign", {
    ...
    body: JSON.stringify({ filename: file.name, contentType: file.type, purpose: "offer-photo" }),
  });
...
export function CreateOfferForm({ minSalePriceCents }: { minSalePriceCents: number }) {
  ...
  const refreshQuote = useCallback(async (dollars: string) => {
    ...
    if (res.ok) {
      ...
      setQuoteNote(
        json?.minSalePriceCents ? `Minimum offer price is ${money(json.minSalePriceCents)}.` : "Enter a valid price."
      );
    }
  }, []);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if ((front && !rear) || (!front && rear)) {
      setError("Add both a front and a rear photo, or leave both out.");
      return;
    }
    setBusy(true);
    try {
      const form = new FormData(e.currentTarget);
      let frontPhotoKey: string | undefined;
      let rearPhotoKey: string | undefined;
      if (front && rear) {
        [frontPhotoKey, rearPhotoKey] = await Promise.all([uploadViaPresign(front), uploadViaPresign(rear)]);
      }
      ...
      // On success the action redirects to the offers list.
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-8 lg:grid-cols-[1fr_320px]">
      <div className="space-y-6">
        <section className="space-y-4 rounded-2xl border border-ink-200/70 bg-white p-6 shadow-soft">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <label className="block sm:col-span-2">
              ...
              <textarea name="description" rows={3} className={input} placeholder="Anything a buyer should know…" />
            </label>
          </div>
          <div>
            <p className="mb-2 text-[13px] font-semibold text-ink-700">
              Photos <span className="font-normal text-ink-400">(optional, but they build buyer trust)</span>
            </p>
        ...
        <section className="space-y-4 rounded-2xl border border-ink-200/70 bg-white p-6 shadow-soft">
          <h2 className="text-[15px] font-bold text-ink-900">Offer price</h2>
          <label className="block sm:max-w-xs">
            <span className="mb-1.5 block text-[13px] font-semibold text-ink-700">Asking price (USD)</span>
            <input
              name="salePrice"
              type="number"
              step="0.01"
            ...
            <span className="mt-1.5 block text-xs text-ink-400">
              Minimum {money(minSalePriceCents)}. The first buyer to pay this price wins.
            </span>
          </label>
        </section>
        ...
        >
          {busy ? "Posting offer…" : "Post open offer & get link"}
        </button>
      </div>
      ...
      <aside className="h-fit rounded-2xl border border-brand-200/80 bg-gradient-to-b from-brand-50/80 to-white p-5 shadow-soft lg:sticky lg:top-24">
        <h3 className="mb-3 text-[15px] font-bold text-brand-900">Buyer sees</h3>
        {quote ? (
          ...
          <p className="text-sm text-brand-800/70">{quoteNote ?? "Enter an asking price to preview the numbers."}</p>
        )}
        <p className="mt-4 text-xs leading-relaxed text-brand-800/60">
          Share one link. The first buyer to pay wins; everyone else can join the waitlist in case it re-opens.
        </p>
      </aside>
    </form>
  );
