  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Check in inbound packages and document each card.</p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <form action={checkInAction} className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-teal-500"
          />
          <button className="rounded-lg bg-teal-600 px-5 py-2 text-sm font-semibold text-white hover:bg-teal-700">
            Open deal
          </button>
        </form>
      </section>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white divide-y divide-slate-100">
          {deals.map((d) => (
            <Link
              key={d.id}
              href={`/hub/deals/${d.id}`}
              className={`flex items-center justify-between gap-4 px-4 py-3 hover:bg-slate-50 first:rounded-t-2xl last:rounded-b-2xl ${
                highlight ? "bg-teal-50/40" : ""
              }`}
            >
              <div className="min-w-0">
                <p className="font-semibold text-slate-900 truncate">{cardTitle(d)}</p>
                <p className="text-xs text-slate-400">{d.shortCode}</p>
              </div>
