  ...
  return (
    <section className="rounded-xl border border-teal-200 bg-teal-50/40 p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-teal-900">✔ Verified &amp; documented at the hub</h2>
        {inspection.tamperSealSerial ? (
          <span className="text-xs text-teal-800 font-mono">seal #{inspection.tamperSealSerial}</span>
        ) : null}
      </div>
      {purged ? (
        <p className="text-sm text-slate-500">Inspection video was purged 30 days after delivery.</p>
      ) : (
        <div className="grid sm:grid-cols-[2fr_1fr] gap-3">
          {videoUrl ? (
            <video controls className="w-full rounded-lg border border-teal-200 bg-black aspect-video">
              <source src={videoUrl} />
            </video>
          ) : null}
          <div className="grid grid-cols-2 sm:grid-cols-1 gap-3">
            {photoUrls.map((url, i) => (
              ...
              <img key={i} src={url} alt={`Hub reference ${i + 1}`} className="w-full rounded-lg border border-teal-200 object-cover aspect-square" />
            ))}
          </div>
        </div>
      )}
      {inspection.notes ? <p className="mt-3 text-xs text-slate-500">Notes: {inspection.notes}</p> : null}
    </section>
  );
}
